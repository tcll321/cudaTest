...
#define	CLA10(x) (min(max(red,0.0f),1023.f))
...
__device__ uint32 RGBPACK_8bit(float red, float green, float blue)
{
	uint32 ARGBpixel = 0;
...
extern "C"
__global__ void NV12ToRGB_drvapi(uint32 *srcImage, size_t nSourcePitch,
	uint32 *dstImage, size_t nDestPitch,
	uint32 width, uint32 height)
{
	int32 x, y;
	...
	uint32 dstImagePitch = nDestPitch >> 2;
	uint8 *srcImageU8 = (uint8 *)srcImage;
	uint24 *dstImageU24 = (uint24 *)dstImage;
	...
	dstImageU24[y * dstImagePitch + x].b = Cloamp_10bit(blue[0]);
	dstImageU24[y * dstImagePitch + x].g = Cloamp_10bit(green[0]);
	dstImageU24[y * dstImagePitch + x].r = Cloamp_10bit(red[0]);
	dstImageU24[y * dstImagePitch + x + 1].b = Cloamp_10bit(blue[1]);
	dstImageU24[y * dstImagePitch + x + 1].g = Cloamp_10bit(green[1]);
	dstImageU24[y * dstImagePitch + x + 1].r = Cloamp_10bit(red[1]);
}
